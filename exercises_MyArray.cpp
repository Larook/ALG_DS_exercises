...
template<typename T>
void MyArray<T>::string_reversal() {
...
template<typename T>
void MyArray<T>::merge_sorted_arrays() {
    /*
     ...
     */
    int arr1[] = {1, 3, 4, 6, 8};
    MyArray<int> myArr1 = MyArray<int>(arr1, sizeof(arr1) / sizeof(arr1[0]));

    int arr2[] = {2, 4, 5, 9};
    MyArray<int> myArr2 = MyArray<int>(arr2, sizeof(arr2) / sizeof(arr2[0]));
...
template<typename T>
vector<int> MyArray<T>::two_sum(vector<int> nums, int target) {
    /*
...
template<typename T>
int MyArray<T>::maximum_sum_subarray(vector<int> nums) {
    /* {5, 4, -1, 7, 8}
...
//    return max_subarray_rec(nums, 0, nums.size()-1);
    return MyArray<T>::max_subarray_rec(nums, 0, a - 1);
...
template<typename T>
vector<int> MyArray<T>::rotate_array(vector<int>A, int k){
    /*
